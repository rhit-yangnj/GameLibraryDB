...
where (g.ID = @GameID)  AND
	(@PlatformName is null OR p.[Name] LIKE CONCAT('%', @PlatformName, '%')) 
return @returnString
...
where (g.ID = @GameID)  AND
	(@GenreName is null OR ghg.GenreName LIKE CONCAT('%', @GenreName, '%'))
return @returnString
...
WHERE (@GameName is null OR g.[Name] LIKE CONCAT('%', @GameName, '%')) AND
	...
	(@StudioName is null OR s.[Name] LIKE CONCAT('%', @StudioName, '%')) 
